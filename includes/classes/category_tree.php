...
//        $Qcategories->setCache('category_tree');
        $Qcategories->execute();
    ...
    protected function _buildBranch($parent_id, $level = 0) {
      ...
      if ( isset($this->_data[$parent_id]) ) {
        foreach ( $this->_data[$parent_id] as $category_id => $category ) {
          ...
          $result .= str_repeat($this->spacer_string, $this->spacer_multiplier * $level) . '<a href="' . osc_href_link(null, 'cPath=' . $category_link) . '">' . $link_title . '</a>';
